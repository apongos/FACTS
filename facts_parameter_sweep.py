...
def main(argv):
    ...
    parameter_sweep = list(np.linspace(0.00000001,0.0001, 25).astype(str))
    parameter_sweep = [f'{x:.11}' for x in parameter_sweep]
    print(parameter_sweep)
    for pp in parameter_sweep:
        if pp in parameter_sweep[0:24]:
            continue
        print(pp)
        print(f'##### parameter value {pp}')
        config['SensoryNoise']['Somato_sensor_scale'] = pp
